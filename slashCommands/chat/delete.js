...
module.exports = {
  ...
  run: async ( client, interaction ) => {
    ...
    const { isBotMod, hasManageGuild, hasManageMessages, isWhitelisted, content } = await userPerms( author, guild, true, true );
    if ( content ) { return interaction.editReply( { content: content } ); }

    const canDelete = ( isBotMod || hasManageGuild || hasManageMessages || isWhitelisted ? true : false );
