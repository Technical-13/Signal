...
const modData = { group: 'geocaching', name: 'logo', type: 'slashCommands' };
    ...
    try {
      ...
      if ( msgID && !( /[\d]{18,19}/.test( msgID ) ) ) { return interaction.editReply( { content: '`' + msgID + '` ' + r6e.invalidMsgId[ useLang ] } ); }
        ...
        .then( message => {
          ...
          message.reply( { content: '<@' + msgAuthor.id + '>, ' + r6e.logoInfo[ useLang ] } )
          .then( replied => {
        ...
        channel.send( { content: '<@' + cmdTaggee.id + '>, ' + r6e.logoInfo[ useLang ] } ).then( replied => {
        ...
        interaction.editReply( { content: r6e.logoInfo[ useLang ] } ).catch( noReply => {
          if ( doLogs ) {
