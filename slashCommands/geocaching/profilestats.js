...
const modData = { group: 'geocaching', name: 'profilestats', type: 'slashCommands' };
    ...
    try {
      ...
      .then( sentMsg => {
        if ( doLogs && !isAuthor ) {
          ...
          .catch( async errLog => { await errHandler( errLog, { chanType: 'default', command: modData.name, channel: channel, type: 'logLogs' } ); } );
        }
        else { interaction.deleteReply(); }
      } )
      .catch( async errSend => { interaction.editReply( await errHandler( errSend, { command: modData.name, channel: channel, type: 'errSend' } ) ); } );
    }
