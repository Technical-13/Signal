...
//          trackables: trackables,
          ...
          state: ( ctArr[ ctArr.length - 1 ] === 'disabled' ? 'disabled' : 'active' ),
          terrain: $( '#ctl00_ContentBody_lblTerrain' ).next( 'span' ).text().trim(),
          type: $( '.li__cache-type' ).text().trim()
        };
      }
      else {
        var state = 'active', trackables = 0;
        if ( $( '#ctl00_ContentBody_archivedMessage' ).length ) { state = 'archived' }
        else if ( $( '#ctl00_ContentBody_disabledMessage' ).length ) { state = 'disabled' }
        else if ( $( '#ctl00_ContentBody_lockedMessage' ).length ) { state = 'locked' }
...
//          favorited: 0,
          ...
          type: $( 'svg.icon.cache-icon' ).parents( 'a' ).attr( 'title' ).trim()
        };
      }
      return result;
    } ).catch( errGetPage => {
