module.exports = {
    ...
    const i18FTFinfo = {
      ...
      fi: 'Project-GC tunnistaa FTF-löytöjä kahdella tavalla. Voit merkitä lokisi jollakin seuraavista tunnisteista: {*FTF*} {FTF} [FTF]. Vaihtoehtoisesti voit lisätä FTF-löytösi kirjanmerkkilistaan ja linkittää sen Asetuksissa. Lista tarkistetaan kerran päivässä. Please understand that FTF isn\'t anything offical and not everyone tags their FTFs. Therefore this list wont be 100% accurate.',
      fr: 'Il existe deux manières pour Project-GC de détecter vos FTF. Vous pouvez utiliser un de ces tags dans vos logs : {*FTF*} {FTF} [FTF]. Vous pouvez également ajouter une liste de FTF sous Configuration qui seront vérifiés une fois par jour. Comprenez bien que les FTFs n\'ont rien d\'officiels et que tout le monde ne tague pas ses FTFs. C\'est pourquoi cette liste ne sera pas fiable à 100 %.',
      no: 'Det er to måter Project-GC kan finne FTF-ene dine på. Enten merker du dine logger med en av disse kodene: {*FTF*} {FTF} [FTF]. Eller så kan du legge til en FTF-bokmerkeliste under Innstillinger som vil bli sjekket en gang for dagen. Husk at FTF ikke er noe offisielt, og at ikke alle tagger FTF-ene sine. Derfor vil ikke denne listen være 100 % nøyaktig.',
      pl: 'Project-GC wykrywa wpisy FTF na dwa różne sposoby. Możesz oznaczyć swoje wpisy jednym z tagów: {*FTF*} {FTF} [FTF]. Albo w Ustawieniach możesz wybrać listę zakładek z wpisami FTF, która będzie sprawdzana raz dziennie. Proszę zrozumieć, że FTF nie jest niczym oficjalnym i nie każdy oznacza swoje FTFy. Dlatego ta lista nie jest w 100% dokładna.',
      'sv-SE': 'Det finns två sätt för Project-GC att upptäcka dina FTF:er. Antingen taggar du din logg med någon av dessa taggar: {*FTF*} {FTF} [FTF]. Eller så kan du lägga till en lista med dina FTF:er under Inställningar, den kommer att kontrolleras av sidan en gång per dag. Det är viktigt att inse att FTF inte är en officiell term och att det inte är alla som taggar sina FTF-loggar på vedertaget sätt. Därför kommer denna lista aldrig att vara 100% korrekt.'
    };
    const i18NoMessage = {
