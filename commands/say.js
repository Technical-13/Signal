      ...
      logSchema.findOne( { Guild: interaction.guild.id }, async ( err, data ) => {
        ...
        if ( cmdAllowed ) {
          speakChannel.send( mySaying ).then( async spoke => {
            console.log( 'https://discord.com/channels/%o/%o/%o', spoke.guild.id, spoke.channel.id, spoke.id );// For testing
            logChan.send( 'I spoke in <#' + spoke.channel.id + '> [(:link: link)](https://discord.com/channels/' + spoke.guild.id + '/' + spoke.channel.id + '/' + spoke.id + ') at <@' + interaction.user.id + '>\'s request:\n```\n' + mySaying + '\n```' );
